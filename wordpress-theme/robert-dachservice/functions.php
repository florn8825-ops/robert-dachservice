...
function robert_dachservice_lead_columns(array $columns): array {
    return [
        ...
        'title' => 'Name', 'rd_phone' => 'Telefon', 'rd_email' => 'E-Mail',
        'rd_service' => 'Leistung', 'rd_city' => 'Ort', 'date' => 'Eingegangen'
    ];
}
...
function robert_dachservice_render_lead_meta(WP_Post $post): void {
    ...
    wp_nonce_field('rd_lead_meta','rd_lead_meta_nonce');
    ...
    if(is_array($ids) && $ids){
        ...
        foreach($ids as $id){$url=wp_get_attachment_image_url((int)$id,'medium');if($url) echo '<a target="_blank" href="'.esc_url(wp_get_attachment_url((int)$id)).'"><img style="width:150px;height:110px;object-fit:cover" src="'.esc_url($url).'" alt=""></a>';}
        echo '</div>';
    }
}
...
function robert_dachservice_handle_contact(): void {
    ...
    $title=trim($data['vorname'].' '.$data['nachname']);
    $post_id=wp_insert_post(['post_type'=>'rd_lead','post_status'=>'publish','post_title'=>$title,'meta_input'=>$data],true);
    if(is_wp_error($post_id)) wp_safe_redirect(add_query_arg('kontakt','fehler',wp_get_referer() ?: home_url('/kontakt/'))); else {
        ...
        if(!empty($_FILES['fotos']['name']) && is_array($_FILES['fotos']['name'])){
            ...
            for($i=0;$i<$count;$i++){
                ...
                $file=['name'=>sanitize_file_name($_FILES['fotos']['name'][$i]),'type'=>sanitize_mime_type($_FILES['fotos']['type'][$i]),'tmp_name'=>$_FILES['fotos']['tmp_name'][$i],'error'=>(int)$_FILES['fotos']['error'][$i],'size'=>(int)$_FILES['fotos']['size'][$i]];
                $allowed=['image/jpeg','image/png','image/webp']; if(!in_array($file['type'],$allowed,true)) continue;
                $upload=wp_handle_upload($file,['test_form'=>false,'mimes'=>['jpg|jpeg'=>'image/jpeg','png'=>'image/png','webp'=>'image/webp']]);
                if(isset($upload['file'])){
                    $attachment_id=wp_insert_attachment(['post_mime_type'=>$upload['type'],'post_title'=>sanitize_text_field(pathinfo($file['name'],PATHINFO_FILENAME)),'post_status'=>'inherit'],$upload['file'],$post_id);
                    if(!is_wp_error($attachment_id)){wp_update_attachment_metadata($attachment_id,wp_generate_attachment_metadata($attachment_id,$upload['file']));$ids[]=(int)$attachment_id;}
                }
            }
        }
